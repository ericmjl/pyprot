...
if __name__ == '__main__':
    ...
    for ts, output in zip(target_structures, output_pdbs):
        ...
        # get ligand coordinates and target atoms
        lig_coords = [tar_struc._get_xyz_coords(l) for l in tar_struc.heavy_hetatm]
        target_atoms = set()
        for c in lig_coords:
            target_atoms = target_atoms.union({l for l in tar_struc.grab_radius(radius=args.radius, coordinates=c)})
